...
export default function DailyPaymentsPage() {
  ...
  const entryTotal = useMemo(() => {
    return rows.reduce((sum, row) => sum + (Number(amounts[row.payment_head]) || 0), 0);
  }, [rows, amounts]);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Every active payment head (ACTIVE = Y) is shown in this table for amount entry."
      />
      <Card sx={{ mb: 2 }}>
        <CardContent>
            ...
            <Table size="small">
              ...
              <TableBody>
                {rows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5}>
                      No active payment heads (ACTIVE = Y). Add them under Administration → Payment Heads, or run seed_data.
                    </TableCell>
                  </TableRow>
                ) : (
                  rows.map((row) => (
